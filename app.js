...
// ========================================================
// 1. ربط المشروع بـ Supabase (مشروع بوابة التفعيل الحالي)
// ========================================================
...
// ========================================================
// 2. دالة توليد وقراءة بصمة الجهاز
// ========================================================
...
// ========================================================
// 3. دالة فحص التفعيل والتوجيه لبوابة الاشتراكات
// ========================================================
async function checkActivation() {
    ...
    if (!isCacheValid) {
        showLockScreen("برجاء الضغط على زرار الانتقال لبوابة التفعيل لتنشيط حسابك.");
    }

    // 2. الفحص السحابي الإجباري
    try {
        if (!navigator.onLine) {
        ...
        if (!data || data.length === 0) {
            localStorage.removeItem('contractor_subscription_expiry_cache');
            showLockScreen("🔒 هذا الجهاز غير مسجل أو غير مفعل بالسحابة. جاري توجيهك لصفحة الاشتراك...");
            return false;
        }
        ...
        console.error("فشل الاتصال بالسيرفر أو أوفلاين:", error);
        localStorage.removeItem('contractor_subscription_expiry_cache');
        showLockScreen("⚠️ عذراً، يلزم وجود اتصال بالإنترنت للتحقق من تفعيل الأداة.");
        return false;
    }
}

// دالة إظهار شاشة القفل وتحويل العميل لصفحة الاشتراك
function showLockScreen(msg) {
    ...
    if (!redirectSection) {
        ...
        redirectSection.style.marginTop = '20px';
        redirectSection.style.textAlign = 'center';
        container.appendChild(redirectSection);
    }

    const fingerprint = getDeviceID();
    redirectSection.innerHTML = `
        <div style="background: #1e293b; padding: 15px; border-radius: 12px; border: 1px solid #334155; margin-top: 15px;">
            <p style="font-size: 13px; color: #94a3b8; font-weight: bold; margin-bottom: 12px;">
                💡 لتفعيل الفترة التجريبية (48 ساعة) أو الاشتراك بالباقات، اذهب إلى بوابة التفعيل:
            </p>
            <button onclick="goToSubscriptionPortal('${fingerprint}')" style="background-color: #4f46e5; color: white; padding: 12px 20px; border: none; border-radius: 10px; font-weight: bold; cursor: pointer; width: 100%; font-size: 14px;">
                🚀 الانتقال إلى صفحة التفعيل والدعم
            </button>
        </div>
    `;
}
